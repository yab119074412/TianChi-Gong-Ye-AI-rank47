...
def deal_time(df,date_columns):
    ...
    all_delta_time = pd.DataFrame(delta_time, columns=['time_delta_5'])
    new_time=pd.concat([near_date_data,all_delta_time],axis=1)
    return new_time

if __name__=="__main__":

    TOOL1_data = pd.read_csv('C:/Users/user/Desktop/tool_testA_data/Tool (#5)11.csv').drop('Tool (#5)',axis=1)
    date_columns=[ '520X148', '520X152', '520X171', '520X173', '520X248', '520X250', '520X346', '520X348', '520X354', '520X356']

    # 挑出时间特征
    new_date_columns=deal_time(TOOL1_data,date_columns)
    # print new_date_columns
    new_date_columns.to_csv('C:/Users/user/Desktop/file_testA/tool_6.csv', index=False)

    # all_date=pd.read_csv('C:/Users/user/Desktop/testA_deal_date/train_date1(day).csv')['processing_cycle']
    # for i in range(6):
    #     time_df=pd.read_csv('C:/Users/user/Desktop/file_train/tool_'+str(i+1)+'.csv')
    #     all_date=pd.concat([time_df,all_date],axis=1)
    # all_date.to_csv('C:/Users/user/Desktop/file_train/all_testA_time.csv',index=False)
