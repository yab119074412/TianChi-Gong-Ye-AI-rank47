...
def deal_time(df,date_columns):
    ...
    all_delta_time = pd.DataFrame(delta_time, columns=['time_delta_1'])
    new_time=pd.concat([near_date_data,all_delta_time],axis=1)
    return new_time

if __name__=="__main__":

    TOOL1_data = pd.read_csv('C:/Users/user/Desktop/tool_testA_data/Tool2.csv').drop('Tool',axis=1)
    date_columns=['220X67', '220X71', '220X75', '220X79', '220X83', '220X87', '220X91', '220X95']

    # 挑出时间特征
    new_date_columns=deal_time(TOOL1_data,date_columns)
    new_date_columns.to_csv('C:/Users/user/Desktop/file_testA/tool_2.csv', index=False)
