...
def deal_time(df,date_columns):
    ...
    all_delta_time = pd.DataFrame(delta_time, columns=['time_delta_2'])
    new_time=pd.concat([near_date_data,all_delta_time],axis=1)
    return new_time

if __name__=="__main__":

    TOOL1_data = pd.read_csv('C:/Users/user/Desktop/tool_testA_data/TOOL_ID3.csv').drop('TOOL_ID',axis=1)
    date_columns=['300X2', '300X3', '300X4', '300X6', '300X7', '300X9', '300X10', '300X13', '300X14', '300X20']

    # 挑出时间特征
    new_date_columns=deal_time(TOOL1_data,date_columns)
    new_date_columns.to_csv('C:/Users/user/Desktop/file_testA/tool_3.csv', index=False)
